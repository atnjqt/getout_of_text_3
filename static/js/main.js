document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderTable(data, query = '') {
        ...
        const cols = ['Docket No.', 'Granted', 'Argued', 'Decided', 'url', 'snippet'];
        for (const col of cols) html += `<th>${col === 'snippet' ? 'Context Snippet' : col}</th>`;
        html += '</tr></thead><tbody>';
        for (const row of data) {
            html += '<tr>';
            for (const col of cols) {
                if (col === 'url') {
                    html += `<td><a href="/pdftext?url=${encodeURIComponent(row[col])}">View PDF</a></td>`;
                } else if (col === 'snippet') {
                    let snippet = row[col] || '';
                    if (query) {
